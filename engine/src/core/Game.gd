extends Control
## Game — the chapter-based adventure loop (TFS Visual Novel engine).
##
...
func _build_title_layer() -> void:
	...
	var full := _chapters.game_title if _chapters.game_title != "" else "UNTITLED ADVENTURE"
	var main_line := full
	var sub_line := ""
	var dash := full.find("—")
	if dash != -1:
