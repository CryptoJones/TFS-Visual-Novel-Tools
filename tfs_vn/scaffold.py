...
def _generate_project_data(config: dict[str, Any], config_dir: Path) -> dict[str, Any]:
    ...
    for ch_index, chapter in enumerate(chapters_in, start=1):
        ...
        if chapter.get("require_scenes", config.get("require_scenes", False)):
            quests[quest_id]["steps"] = _scene_steps(quests[quest_id].get("steps", []), rooms, npc_defs)
            _lock_rooms_behind_scenes(rooms, npc_defs)
...
def _scene_steps(steps: list[Any], rooms: dict[str, Any], npc_defs: dict[str, Any]) -> list[dict[str, Any]]:
    """Quest steps plus one "play this scene" objective per conversation.

    Opt-in with ``require_scenes`` (project-wide or per chapter), for games
    where the conversations ARE the story: reaching the last room then no longer
    finishes a chapter on its own. Each objective is keyed to ``heard_<npc id>``,
    which the engine sets when a conversation reaches its end, so only NPCs
    whose dialog can actually end are required. Steps follow the rooms in
    order, each room-entry step just ahead of the scenes in its room, so the
    HUD objective names the next unplayed scene. A step already authored for a
    ``heard_`` flag keeps its wording and takes that scene's place.
    """
    authored = [dict(step) for step in steps if isinstance(step, dict)]
    result: list[dict[str, Any]] = []
    placed: set[int] = set()

    def place(flag: Any) -> bool:
        hits = [i for i, step in enumerate(authored) if i not in placed and flag and step.get("flag") == flag]
        result.extend(authored[i] for i in hits)
        placed.update(hits)
        return bool(hits)

    for room in rooms.values():
        place(room.get("on_enter_flag"))
        for npc_id in room.get("npcs", []):
            flag = f"heard_{npc_id}"
            if place(flag) or not _dialog_can_end(npc_defs, npc_id) or any(step.get("flag") == flag for step in result):
                continue
            name = str(npc_defs[npc_id].get("name") or titleize_slug(npc_id))
            result.append({"text": f"Talk: {name} ({room['name']})", "flag": flag})
    result += [step for index, step in enumerate(authored) if index not in placed]
    return result


def _dialog_can_end(npc_defs: dict[str, Any], npc_id: str) -> bool:
    nodes = as_object(npc_defs.get(npc_id, {}).get("nodes"), f"{npc_id}.nodes")
    return any(not node.get("options") for node in nodes.values() if isinstance(node, dict))


def _lock_rooms_behind_scenes(rooms: dict[str, Any], npc_defs: dict[str, Any]) -> None:
    """With every scene required, the story has to be walked in order.

    Each room is locked until the last scene of the room before it has been
    heard, and every room keeps a way back. Without this, a reader who walks
    ahead of a required scene into a room with no exit back has an unfinishable
    chapter. An authored requires_flag or exit is never overwritten.
    """
    ids = list(rooms)
    for prev_id, room_id in zip(ids, ids[1:]):
        prev, room = rooms[prev_id], rooms[room_id]
        scenes = [npc_id for npc_id in prev.get("npcs", []) if _dialog_can_end(npc_defs, npc_id)]
        if scenes and "requires_flag" not in room:
            name = str(npc_defs[scenes[-1]].get("name") or titleize_slug(scenes[-1]))
            room["requires_flag"] = f"heard_{scenes[-1]}"
            room["locked_text"] = f"Not yet \u2014 talk first: {name}."
        if prev_id not in room["exits"].values():
            room["exits"].setdefault("west", prev_id)
