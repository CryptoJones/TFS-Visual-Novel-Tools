...
def _scene_steps(steps: list[Any], rooms: dict[str, Any], npc_defs: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for room in rooms.values():
        ...
        for npc_id in room.get("npcs", []):
            ...
            can_end = any(not node.get("options") for node in nodes.values() if isinstance(node, dict))
            if place(flag) or not can_end or any(step.get("flag") == flag for step in result):
                continue
